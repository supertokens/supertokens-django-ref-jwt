...
def encrypt(plaintext, masterkey):
    ...
    key = md5(b64encode(pbkdf2_hmac("sha256", masterkey.encode(), salt, 32, 100))).hexdigest()
...
def assert_user_id_has_correct_format(user_id):
    if type(user_id) != str and type(user_id) != int:
...
def serialize_user_id(user_id):
    ...
    if type(user_id) == str:
        try:
            ...
            if type(json_from_user_id) == dict and len(json_from_user_id) == 1 and 'i' in json_from_user_id:
                raise_general_exception("passed userId cannot be stringified version of object type {i: string}")
...
def unserialize_user_id(user_id):
    try:
        id = loads(user_id)
        if type(id) == dict and len(id) == 1 and 'i' in id:
            return id['i']
    except:
        pass
    return user_id