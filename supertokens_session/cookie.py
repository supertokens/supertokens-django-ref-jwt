...
def get_anti_csrf_header(request):
    get_header(request, ANTI_CSRF_HEADER_KEY)

def clear_session_from_cookie(response):
    set_cookie(response, ACCESS_TOKEN_COOKIE_KEY, '',  datetime.fromtimestamp(0), supertokens_settings.ACCESS_TOKEN_PATH, supertokens_settings.COOKIE_DOMAIN, supertokens_settings.COOKIE_SECURE, True)
    set_cookie(response, ID_REFRESH_TOKEN_COOKIE_KEY, '',  datetime.fromtimestamp(0), supertokens_settings.ACCESS_TOKEN_PATH, supertokens_settings.COOKIE_DOMAIN, False, False)
    set_cookie(response, REFRESH_TOKEN_COOKIE_KEY, '',  datetime.fromtimestamp(0), supertokens_settings.REFRESH_TOKEN_PATH, supertokens_settings.COOKIE_DOMAIN, supertokens_settings.COOKIE_SECURE, True)
